...
public:
    ...
    void advance(double dt) {
        const size_t N = (bodies.size() - 1)*bodies.size() / 2;
        struct alignas(16) R {
            double dx, dy, dz, filler;
        };
        alignas(16) static R r[1000];
        alignas(16) static double mag[1000];

        for (size_t i = 0, k = 0; i < bodies.size() - 1; ++i) {
            Body& iBody = bodies[i];
            for (size_t j = i + 1; j < bodies.size(); ++j, ++k) {
        ...
        for (size_t i = 0; i < N; i += 2) {
            ...
            for (size_t j = 0;j < 2;++j)
            {
                distance = _mm_sub_pd( _mm_mul_pd(distance, _mm_set1_pd(1.5)),
                    _mm_mul_pd(_mm_mul_pd(_mm_set1_pd(0.5), dSquared), _mm_mul_pd(_mm_mul_pd(distance, distance), distance)));
            }
